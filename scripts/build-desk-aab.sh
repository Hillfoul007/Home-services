#!/bin/bash
# Build signed AAB for Laundrify Desk app
# Usage: npm run build:aab:desk
...
echo "🖥️  Building Desk App Bundle (AAB)..."
echo ""
...
# Step 3: Create desk-specific dist with hash-based route redirect
echo "📦 Step 2/5: Preparing desk web assets..."
rm -rf "$DIST_DIR"
...
echo "📦 Step 3/5: Configuring Android project for Desk..."
cp capacitor.config.ts capacitor.config.ts.bak
cp capacitor.desk.config.ts capacitor.config.ts
...
# Update strings.xml with desk app name and package (AFTER cap sync — it overwrites strings.xml)
...
if [ -f "$AAB_FILE" ]; then
  cp "$AAB_FILE" "dist-aab/laundrify-desk-release.aab"
  echo ""
  echo "✅ Desk AAB built successfully!"
  echo "   📱 Output: dist-aab/laundrify-desk-release.aab"
  echo "   📋 App ID: ${APP_ID}"
